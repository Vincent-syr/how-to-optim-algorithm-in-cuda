...
def test_transpose():   
    ...
    output = torch.full((num_pages, (page_cache_bytes + page_scale_bytes) * tp_size), 0, dtype=torch.int8).cuda()
    transpose_v0 = load(name='kv_cache_transpose', sources=['transpose_v0_2.cu'], extra_cuda_cflags=['-O2'])
    # for page_seq, page_id in enumerate(page_list):
    for i in range(0, len(page_list), step_num_pages):
        output_start_page = i
        page_ids = torch.tensor(page_list[i:i+step_num_pages], dtype=torch.int64).cuda()

        for tp_rank in range(tp_size):
            kv_cache = kv_cache_list[tp_rank]
            kv_scale = kv_scale_list[tp_rank]
            output_tmp = transpose_v0.ReorderLayout3(kv_cache, kv_scale, page_ids, tp_rank, tp_size, output_start_page, output)
            # print("output: \n", output)
...
def test_invert_transpose():
    ...
    for i in range(0, len(page_list), step_num_pages):
        ...
        for tp_rank in range(tp_size):
            ...
            transpose_v0.InvertReorderLayout3(input_mem, page_ids, input_start_page, tp_rank, tp_size, kv_cache, kv_scale)
            # print("output: \n", output)
            torch.cuda.synchronize()
    # print("output_cuda: ", output_tmp.tolist())
    # print(output.data_ptr())
